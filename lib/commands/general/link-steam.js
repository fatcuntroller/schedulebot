...
function mmrUnknownNotifyUser(discordUser) {
	return new Promise((fulfill, reject) => {
		...
		if (cfg.dota.mmr.enforce) {
			msg += "In order to participate in any " + cfg.dota.game_generic_name + " you need to " +
				"display your MMR. In order to do so, just log in OpenDota, while making sure that " +
				"you're exposing your MMR in Dota 2.";
		} else {
			msg += "It is not required to display your MMR, but if you wish to do so, just log in " +
				"OpenDota, while making sure that you're exposing your MMR in Dota 2."
		}

		discordUser.send(msg).then(fulfill).catch(reject);
	});
}