	...
	onReply: (msg, context) => {
		context.msg.reply("\n").then(botResponse => {
			if (cfg.delete_after_reply.enabled) {
				context.msg.delete({timeout: cfg.delete_after_reply.time}).catch(console.error);
				botResponse.delete({timeout: cfg.delete_after_reply.time}).catch(console.error);
			}
		}).catch(console.error);
...
let adminApp = new Clapp.App({
	...
	onReply: (msg, context) => {
		context.msg.reply("\n").then(bot_response => {
			if (cfg.delete_after_reply.enabled) {
				context.msg.delete({timeout: cfg.delete_after_reply.time}).catch(console.error);
				bot_response.delete({timeout: cfg.delete_after_reply.time}).catch(console.error);
			}
		}).catch(console.error);
		...
		if (generalApp.isCliSentence(msg.content)) {
			...
			if (blacklist.indexOf(msg.author.id) === -1) {
				...
				msg.reply('\n' + "Sorry, you are blacklisted and can't use any commands.")
					.then(bot_response => {
					msg.delete();
					bot_response.delete({timeout: 7500});
				});
			}
...
Promise.all(startupPromises).then(values => {
	...
	blacklist = values[2];
	bot.on("ready", () => {
		masterChannel = bot.channels.cache.get(cfg.master_channel);
		//}, 1000);
...
function sendMessage(content) {
	masterChannel.send(content).then(botMessage => {
		if (cfg.delete_after_reply.enabled) {
			botMessage.delete({timeout: cfg.delete_after_reply.time}).catch(console.error);
		}
	}).catch(console.error);
};